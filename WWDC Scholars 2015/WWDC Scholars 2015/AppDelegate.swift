...
//        PFAnalytics.trackAppOpenedWithLaunchOptions(launchOptions)
        ...
        reply: (([NSObject : AnyObject]?) -> Void))
    {
        if let pfqueryRequest: AnyObject = (userInfo as? [String: AnyObject])?["pfquery_request"] {
            print("Starting PFQuery") // won't print out to console since you're running the watch extension
            ...
            do {
                ...
                for object in objects! {
                    let age = object.objectForKey("age") as! Int
            ...
            if let containerURL = NSFileManager.defaultManager().containerURLForSecurityApplicationGroupIdentifier("group.wwdcscholars.2015") {
                let scholarsArray = NSMutableArray()
                let query = PFQuery(className:"scholars")
                do {
                    let objects = try query.findObjects()
                    print("Successfully retrieved \(objects!.count) scholars.")
